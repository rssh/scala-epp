...
trait EppRpcServiceImpl extends EppRpcService {
  ...
  override def hello(): Future[GreetingType] = {
    throw new UnsupportedOperationException("hello() should be implemented separately")
  }

  override def login(loginType: LoginType): Future[GreetingType] = {
    throw new UnsupportedOperationException("login() should be implemented separately")
  }

  override def logout(): Future[ResponseType] = {
    val logoutRecord = DataRecord(None, Some("logout"), "")
        ...
        records.find { record =>
          ...
          case Some(record) =>
            try {
              ...
              case e: Exception =>
                Future.failed(new RuntimeException(s"Failed to extract $elementName from response: ${e.getMessage}", e))
            }
          case None =>
            Future.failed(new RuntimeException(s"Expected response data element '$elementName' not found in namespace $namespace"))
        }
      case None =>
        Future.failed(new RuntimeException("No response data in EPP response"))
    }
  }
}
